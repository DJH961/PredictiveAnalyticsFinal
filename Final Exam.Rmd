...
ets_specs <- list(
    ets_1 = ETS(box_cox(views_in_billions, lambda = guerrero_lambda) ~ error("A") + trend("A") + season("A")),
    ets_2 = ETS(box_cox(views_in_billions, lambda = guerrero_lambda) ~ error("A") + trend("Ad") + season("A")),
...
```{r Residual Plots}
plots <- plot_residuals(arima_full_models)
for (plot in plots) {
  print(plot)
}
```
...
```{r initial_test}
rolling_cv_predictions<- function(train_data, cv_models) {
...
model_names <- names(final_models)[!names(final_models) %in% c(".id", "views_in_billions")]
accuracy_results <- cv_results_trimmed %>%
  accuracy(data = train_data, measures = list(MASE = MASE, MAPE = MAPE))

report_models(final_models, model_names)
...
```{r Rolling Cross-Validation Models}
arima_cv_models <- rolling_resamples %>%
  model(!!!arima_specs)

ets_cv_models <- rolling_resamples %>%
  model(!!!ets_specs)
```

```{r run_cv_models}
cv_results_arima <- rolling_cv_predictions(train_data, arima_cv_models)
```


```{r Plot_residuals}
residuals_arima1 <- final_models %>% 
  select(arima_1) %>%
  gg_tsresiduals()

ggsave("residuals_arima1.png", plot = residuals_arima1, dpi = 300, width = 10, height = 7)
residuals_arima1
...
train_data_filtered <- train_data %>%
  slice(99:n())
autoplot(train_data_filtered, views_in_billions) +
